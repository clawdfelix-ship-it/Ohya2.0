...
module.exports = function(app, pool) {
  ...
  function abortTx(status, message) {
    const err = new Error(message);
    err.httpStatus = status;
    err.httpPayload = { error: message };
    throw err;
  }
  ...
  app.post('/api/admin/purchase-orders', requirePermission('inventory:write'), async (req, res) => {
    try {
      ...
      try {
        ...
        if (skuRows.rows.length !== skuIds.length) abortTx(400, '包含不存在的 sku_id');
        ...
        if (err && err.httpStatus) {
          return res.status(err.httpStatus).json(err.httpPayload);
        }
        throw err;
      } finally {
        client.release();
  ...
  app.post('/api/admin/purchase-orders/:id/receive', requirePermission('inventory:write'), async (req, res) => {
    try {
      ...
      try {
        ...
        if (po.rows.length === 0) abortTx(404, '採購單不存在');
        ...
        for (const line of lines) {
          ...
          if (poi.rows.length === 0) abortTx(400, `採購單未包含 SKU #${line.sku_id}`);

          const item = poi.rows[0];
          const maxQty = Number(item.quantity);
          const receivedQty = Number(item.received_quantity || 0);
          if (receivedQty + line.quantity > maxQty) abortTx(400, `SKU #${line.sku_id} 收貨數量超過採購數量`);
          ...
          if (skuRow.rows.length === 0) abortTx(400, `SKU #${line.sku_id} 不存在`);
        ...
        if (txErr && txErr.httpStatus) {
          return res.status(txErr.httpStatus).json(txErr.httpPayload);
        }
        throw txErr;
      } finally {
        client.release();
