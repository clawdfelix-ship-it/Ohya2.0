...
module.exports = function(app, pool) {
    ...
    try {
      ...
      try {
        ...
        if (skuRows.rows.length !== skuIds.length) {
          return rollbackJson(client, res, 400, { error: '包含不存在的 sku_id' });
        }
  ...
  app.post('/api/admin/purchase-orders/:id/receive', requirePermission('inventory:write'), async (req, res) => {
    try {
      ...
      try {
        ...
        if (po.rows.length === 0) return rollbackJson(client, res, 404, { error: '採購單不存在' });
        ...
        for (const line of lines) {
          ...
          if (poi.rows.length === 0) {
            return rollbackJson(client, res, 400, { error: `採購單未包含 SKU #${line.sku_id}` });
          }
          ...
          if (receivedQty + line.quantity > maxQty) {
            return rollbackJson(client, res, 400, { error: `SKU #${line.sku_id} 收貨數量超過採購數量` });
          }
          ...
          if (skuRow.rows.length === 0) {
            return rollbackJson(client, res, 400, { error: `SKU #${line.sku_id} 不存在` });
          }
  ...
  app.post('/webhooks/fps-payme', async (req, res) => {
    try {
      ...
      // Success should mark the order paid once; failed callbacks must not erase a paid state.
      if (status === 'success') {
        await pool.query(
          `UPDATE orders
           SET payment_status = 'paid', payment_transaction_id = $1, paid_at = COALESCE(paid_at, NOW())
           WHERE id = $2`,
        ...
        await pool.query(
          `UPDATE orders
           SET payment_status = 'failed', payment_transaction_id = COALESCE(payment_transaction_id, $1)
           WHERE id = $2 AND COALESCE(payment_status, '') <> 'paid'`,
          [transaction_id, order_id]
        );
      }
